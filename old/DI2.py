...
for searchpatternDict in ListOfsearchpatternDict:
    ...
    if len(foundusermoviescoresList) > 1:
        AATR = sorted(foundusermoviescoresList, key=itemgetter('Total Rating'))[-2]
        ListAATR.append(AATR['Total Rating']) 

        
        AAU = sorted(foundusermoviescoresList, key=itemgetter('Users'))[-2]
        ListAAU.append(AAU['Users'])

        
        AAAR = sorted(foundusermoviescoresList, key=itemgetter('Ave Rating'))[-2]
        ListAAAR.append(AAAR['Ave Rating'])

        
        AAPOU = sorted(foundusermoviescoresList, key=itemgetter('Per of Users'))[-2]
        ListAAPOU.append(AAPOU['Per of Users']) 

        
        AANAR = sorted(foundusermoviescoresList, key=itemgetter('Norm Ave Rating'))[-2]
        ##for row in newlist:
        ##    print (row['item_id'], row['Total Rating'], row['Users'], row['Ave Rating'], row['Per of Users'])

        #print AATR

        ListAANAR.append(AANAR['Norm Ave Rating']) 
...
ListsOfLists = [ListAATR, ListAAU, ListAAAR, ListAAPOU, ListAANAR]
...
for uuu in range(0, len(basic_histo_types)):
    ...
    nbins = 100
    xmin = ListsOfLists[uuu][0]
    xmax = ListsOfLists[uuu][-1]+1
    Captions = MovieName + ", Match" + SearchStragedy + " Stars, "  + basic_histo_types[uuu] + ";" + basic_histo_types[uuu] + "; Count"
    Names = MovieName + ", Match" + SearchStragedy + ", " + basic_histo_types[uuu]
    ...
    for ttt in range(0,len(ListsOfLists[uuu])):
        h_xxx.Fill(ListsOfLists[uuu][ttt])
...
##    if uuu == 0:
##        h_AATR = TH1D(Names, Captions, nbins, xmin, xmax)
##        h_AATR.SetFillStyle(fills[uuu])
##        h_AATR.SetFillColor(colors[uuu])
##    elif uuu == 1:
##        h_AAU = TH1D(Names, Captions, nbins, xmin, xmax)
##        h_AAU.SetFillStyle(fills[uuu])
##        h_AAU.SetFillColor(colors[uuu])
##    elif uuu == 2:
##        h_AAAR = TH1D(Names, Captions, nbins, xmin, xmax)
##        h_AAAR.SetFillStyle(fills[uuu])
##        h_AAAR.SetFillColor(colors[uuu])
##    elif uuu == 3:
##        h_AAPOU = TH1D(Names, Captions, nbins, xmin, xmax)
##        h_AAPOU.SetFillStyle(fills[uuu])
##        h_AAPOU.SetFillColor(colors[uuu])
##    elif uuu == 4:
##        h_AANAR = TH1D(Names, Captions, nbins, xmin, xmax)
##        h_AANAR.SetFillStyle(fills[uuu])
##        h_AANAR.SetFillColor(colors[uuu])
